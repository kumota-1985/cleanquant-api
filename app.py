...
def _safe_symbol(symbol: str) -> str:
    """ユーザー指定 symbol を検証してからパス要素に使う(パストラバーサル防止)。英数字のみ許可。
    fullmatch を使う(`$` は末尾改行の直前にもマッチするため "BTCUSDT\\n" 等の混入を防げない)。"""
    s = (symbol or "").upper()
    if not _SYMBOL_RE.fullmatch(s):
        raise HTTPException(status_code=422, detail="symbol must be alphanumeric, e.g. BTCUSDT")
    return s
...
def funding(symbol: str = Query(...),
    ...
    df = _read("crypto", _safe_symbol(symbol), "funding.parquet")[["time", "fundingRate", "symbol"]]
